...
- 支持编辑或取消尚未发送的插队/排队消息；已完成轮次的问题也可修改，并从该节点重新生成后续对话。
- 附件缩略图、名称和图片内容可随会话恢复；增强多图片、Finder 图片以及图文混合剪贴板粘贴。
- 新增原生多问题问卷界面，完整支持 Plan Mode 等扩展发起的结构化选择和自定义回答。
- 新增跟随系统、浅色和深色主题，以及全局 `~/.pi/agent/AGENTS.md` 个性化编辑器。
- Provider 设置改为双列品牌卡片，加入真实厂商图标、统一认证按钮和逐模型推理强度配置。
- 重构 Git 操作弹窗、会话工具栏和消息布局，统一设置与 Git 界面的 `15pt` 字体和扁平化控件。
...
- 默认情况下，全局快捷键只会打开窗口并聚焦输入框。可在“设置 > 通用”中开启“快捷键打开时填入新复制的文本”；开启后，在其他应用中复制新文本再按快捷键，会将文本填入输入框但不会自动发送。
- 输入框支持多行内容并会随文本自动增高，按 `Shift + Return` 换行，按 `Return` 发送。
- 支持流式 Markdown、代码块复制、停止生成，以及回答区域展开或收起。
- 回答生成期间可以继续编辑下一条问题，并选择“插队”在当前工具调用结束后优先发送，或选择“排队”在当前回答完成后发送；尚未交付的消息可继续编辑或取消。
- 工具调用按回答阶段分组显示，运行时保持展开，完成后可折叠查看输入与输出。
- 每个问题和回答都可单独复制；已完成的问题可修改并从该节点重新生成后续对话，任意已完成回答也可克隆为新会话。
- 可设置登录后自动启动、首页系统状态显示，以及主窗口失焦后是否自动隐藏。
- 在“设置 > 个性化”中切换跟随系统、浅色或深色主题，并直接编辑 Pi 的全局 `AGENTS.md` 指令。

### 模型与 Provider

- 使用带真实厂商图标的双列 Provider 列表，并按 Provider 能力选择 API Key 或 OAuth 登录。
- 添加、编辑或删除 OpenAI Responses / Anthropic Messages 兼容的自定义 Provider。
- 从 Provider 的 `/models` 接口同步模型列表，并记住当前模型。
- 可为自定义 Provider 的每个模型声明支持的推理强度，在主窗口快速切换模型和当前推理级别；图片只会提交给支持图片输入的模型。

### 工作区与 Agent 工具

- 选择并记住一个项目工作区，Pi 会以该目录作为真实运行目录。
- 顶部展示当前会话的 Git 分支；点击后可在扁平化 Git 面板中提交、提交并推送、推送、新建或切换本地分支，并查看 Diff 与最近 Log。
- 右上角“新建会话”按钮和 `/new` 只在当前运行目录中新建会话，不会执行 Git 操作。
...
- 支持图片、PDF、DOCX、常见文本格式及代码文件；也可直接粘贴剪贴板中的一张或多张图片，以及 Finder 中复制的图片文件。
- 图文混合剪贴板会同时保留文本和图片，图片附件会在输入区和历史问题中显示缩略图。
- 每次最多添加 5 个附件，每个文件不超过 10 MB，单个文本内容不超过 200,000 字符。
- 图片会在本地转换为 JPEG，并将最长边限制为 2,048 像素；剪贴板位图会先转换，再按 10 MB 上限校验。
- PDF、DOCX 与文本文件会先在本地提取文字，再随问题发送给所选模型。
- 附件元数据和图片内容随会话保存，切换会话或重新启动后仍可恢复查看。
...
- 规划完成后，原生界面会显示“执行、保持规划或继续调整”的选择；扩展需要澄清多个问题时，会打开支持推荐项、跳过和自定义回答的原生问卷；执行期间会继续展示步骤和完成进度。
...
5. 输入问题并按 Return 发送。默认情况下，全局快捷键只会打开 Quick Pi；也可在“设置 > 通用”中开启剪贴板文本自动填入，填入后仍需手动发送。
...
Pi 内置 Provider 的凭证、全局设置、`AGENTS.md` 和已安装 package 仍位于 `~/.pi/agent/`，因此终端 Pi 与 Quick Pi 会共用这些全局能力。全局 `AGENTS.md` 可在“设置 > 个性化”中编辑，保存后会用于后续新请求。应用会将自身设置、模型配置与凭证文件权限设为仅当前用户可读写；这些文件仍包含敏感信息，请勿提交或分享。
