...
final class ConfigurationStoreTests: XCTestCase {
    ...
    // Verifies that terminal access can never be persisted without its inherent file-system access.
    func testInvalidTerminalPermissionCombinationIsRejected() throws {
        let directory = try temporaryDirectory()
        let store = ConfigurationStore(applicationSupportDirectory: directory)
        ...
        try JSONEncoder().encode(invalid).write(to: settingsURL)
        XCTAssertThrowsError(try store.load())
    }
